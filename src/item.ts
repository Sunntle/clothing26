...
async function loadItem() {
  ...
  function loadDataItem() {
    ...
    html = `<form name="fmk1">
            ...
            ${data.img
              .map(
                (el, index) =>
                  (imgString = `<p><span style="width:200px">${el}</span>: <img class="img-fluid bg-transparent px-3"  style="max-width: 100px" src="../asset/img/${el}"><button type="button" class="btn-close" id="btnClose" data-value="${index}" aria-label="Close"></button></p>`)
              )
              .join("")}
            </p> 
            <div class="mb-3">
  ...
  const btnCloseAll: NodeListOf<Element> =
  ...
  function getForm() {
    ...
    if (hinhAnh) data.img?.push(hinhAnh);
    else if (img.value) data.img?.push(img.value.toString());
    data1.img = [...data.img];
