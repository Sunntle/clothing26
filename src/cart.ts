...
const renderCart = () =>{
    ...
    //delete
    const btnDelete: NodeListOf<Element> = document.querySelectorAll('.btn-delete')
    ...
    //quantity
    const quantity:NodeListOf<Element> = document.querySelectorAll('.quantity button')
    quantity.forEach(el =>
        el.addEventListener('click', function (e) {
            ...
            if(button){
                let oldValue = button?.parentElement?.parentElement?.querySelector('input')
                let newVal = 0
                if(oldValue){
