...
async function loadData() {
  ...
  btnAddNew?.addEventListener("click", (e) => {
    ...
    let sizeM = document.querySelector(
      'input[name="sizeM"]'
    ) as HTMLInputElement;
    let sizeL = document.querySelector(
      'input[name="sizeL"]'
    ) as HTMLInputElement;
    let sizeXL = document.querySelector(
      'input[name="sizeXL"]'
    ) as HTMLInputElement;
    ...
    if (flag) {
      ...
      if (hinhAnh) data1.img?.push(hinhAnh);
      else data1.img?.push(img.value);
      if (date.value) {
