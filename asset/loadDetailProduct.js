...
fetch(url)
    ...
    responsiveNav();
    if (product !== null) {
        let imgItem = [], sizeItem = [], colorItem = [];
        ...
        document.querySelector("#img-product").innerHTML =
            imgItem.join("");
        const productName = document.createElement("h3");
        productName.classList.add("product-name");
        productName.textContent = `${product.name}`;
        document.querySelector("#detailsProduct").appendChild(productName);
        ...
        document.querySelector("#detailsProduct").appendChild(rateStar);
        const productPrice = document.createElement("h3");
        productPrice.classList.add("font-weight-semi-bold", "mb-4");
        productPrice.textContent = `${product.price}`;
        document.querySelector("#detailsProduct").appendChild(productPrice);
        ...
        document.querySelector("#detailsProduct").appendChild(productAvailable);
        ...
        document.querySelector("#detailsProduct").appendChild(productDescription);
        if (product.size) {
            ...
            document.querySelector("#detailsProduct").appendChild(divSize);
        ...
        document.querySelector("#detailsProduct").appendChild(divColor);
        ...
        document.querySelector("#detailsProduct").appendChild(divCount);
        ...
        document.querySelector("#detailsProduct").appendChild(divShare);
        if (document.querySelector("#formSize"))
            document.querySelector("#formSize").innerHTML =
                sizeItem.join("");
        document.querySelector("#formColor").innerHTML =
            colorItem.join("");
        const amoutProduct = document.querySelector(".productAvailable");
        if (product.size) {
            const sizes = Object.keys(product.size);
            for (const size of sizes) {
                const sizeNew = document.querySelector(`#size-${size}`);
                    ...
                    if ((_a = e.target) === null || _a === void 0 ? void 0 : _a.checked) {
                        ...
                        if (sizeObj) {
                            ...
                            for (const [key, value] of entries) {
                                if (key === size) {
                                    const amount = (_b = value.amount) !== null && _b !== void 0 ? _b : 0;
        ...
        document.querySelectorAll(".quantity button").forEach((el) => el.addEventListener("click", function (e) {
            var _a, _b;
            const event = e.target || null;
            const button = event.tagName === "BUTTON" ? event : event.parentElement;
            if (button) {
                let oldValue = (_b = (_a = button === null || button === void 0 ? void 0 : button.parentElement) === null || _a === void 0 ? void 0 : _a.parentElement) === null || _b === void 0 ? void 0 : _b.querySelector("input");
                let newVal = 0;
                if (oldValue) {
        ...
        const colorClicked = document.getElementsByName("color");
        let colorItemClicked, sizeItemClicked = "";
        colorClicked.forEach((color) => {
            color.addEventListener("input", function (e) {
                colorItemClicked = e.target.value;
            });
        });
        if (product.size) {
            const sizeClicked = document.getElementsByName("size");
            sizeClicked.forEach((size) => {
                size.addEventListener("input", function (e) {
                    sizeItemClicked = e.target.value;
                });
            });
            if (!sizeItemClicked)
                sizeItemClicked = sizeClicked[0].value;
        }
        const btnAddToCart = document.querySelector("#shopping-cart");
        btnAddToCart.addEventListener("click", (e) => {
            const { id, name, img, price } = product;
            const quantityItem = document.querySelector(".quantity input").value;
            if (!colorItemClicked)
                colorItemClicked = "Black";
            if (!sizeItemClicked)
                sizeItemClicked = "M";
            addtocart(id, name, img[0], colorItemClicked, price, +quantityItem, sizeItemClicked);
        });
